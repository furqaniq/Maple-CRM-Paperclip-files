...
- **This trail is the administrative record, and it is not the compliance one.** AEGIS's `audit-ledger` holds the immutable record of outbound content, gate decisions, and consent. The two are separate records with separate scopes, and neither substitutes for the other — an auditor asking what was sent to a contact is answered from AEGIS's ledger, and one asking who granted the access that sent it is answered from here.
