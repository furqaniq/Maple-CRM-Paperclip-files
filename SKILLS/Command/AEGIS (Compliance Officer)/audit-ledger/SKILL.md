...
- **This ledger is the compliance record of outbound content and consent, and it is not the administrative one.** WARDEN's `admin-audit-trail` records who changed what in the workspace — provisioning, permissions, structure, module toggles. The two are separate records with separate scopes, and neither is ever served in place of the other: an export answering what the company sent and on what consent comes from here, and one answering who changed a permission comes from WARDEN.
