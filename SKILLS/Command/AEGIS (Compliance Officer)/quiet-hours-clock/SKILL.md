...
- **AEGIS holds the cap and the roster-wide count, and enforces both.** AEGIS holds what the ceiling is per channel, window, and jurisdiction, holds the contact-frequency count across every agent that reaches a contact, and evaluates every send in front of both. Contact-frequency enforcement across the platform has exactly one enforcement point and this is it — a roster-wide rule enforced in two places is enforced twice or not at all, and not at all is the outcome that ships silently.
- **RELAY's suppression is a narrower, second check and never a substitute for this one.** RELAY's `cross-campaign-suppression` counts sends within the campaign surface and decides which of its own campaigns yields when two would collide; VOX reads the roster-wide voicemail counter held here before every drop. Both run, neither replaces the gate, and no agent assumes another already applied the platform-wide cap. Where a send's frequency state is unreadable, this skill blocks rather than evaluating against a partial history.
